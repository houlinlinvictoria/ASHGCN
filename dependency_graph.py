...
def dependency_adj_matrix(text_left,aspect,text_right,k,m):
    ...
    fname = "./aspect_xuni_rest15.txt"
    fin = open(fname, 'r', encoding='GBK', newline='\n', errors='ignore')
        ...
        if token_str == aspect:
            ...
            for attention in words:
                if attention in aspect_linear_right:
                    ...
                    matrix1[r][d] = 1-(d+m-r)/len(words)
                    matrix1[d][r] = 1-(d+m-r)/len(words)
            for attention in words:
            ...
            for attention in words:
                if attention in aspect_linear_right_right:
                    d=words.index(attention)
                    matrix1[r][d] = 1-(r-d)/len(words)
                    matrix1[d][r] = 1-(r-d)/len(words)
            for attention in words:
...
if __name__ == '__main__':
    ...
    #process('./datasets/covid-2019/covid-2019-en_train.raw')
    #process('./datasets/covid-2019/covid-2019-en_test.raw')
